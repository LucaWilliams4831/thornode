...
func getWithdrawTestKeeper2V84(c *C, ctx cosmos.Context, k keeper.Keeper, runeAddress common.Address) keeper.Keeper {
...
func (WithdrawSuiteV84) TestWithdrawAsym(c *C) {
	...
	for _, tc := range testCases {
		c.Logf("name:%s", tc.name)
		ctx, mgr := setupManagerForTest(c)
		ps := getWithdrawTestKeeper2V84(c, ctx, mgr.Keeper(), runeAddress)
		mgr.K = ps
