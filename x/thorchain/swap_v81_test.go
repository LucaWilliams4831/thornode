...
func (s *SwapV81Suite) TestSwap(c *C) {
	...
	for _, item := range inputs {
		...
		amount, evts, err := newSwapperV81().Swap(ctx, poolStorage, tx, item.target, item.destination, item.tradeTarget, "", "", nil, cosmos.NewUint(1000_000), 2, mgr)
...
func (s *SwapV81Suite) TestSynthSwap_RuneSynthRune(c *C) {
	...
	// swap rune --> synth
	{
		...
		amount, _, err := newSwapperV81().Swap(ctx, mgr.Keeper(), tx, common.BNBAsset.GetSyntheticAsset(), addr, cosmos.ZeroUint(), "", "", nil, cosmos.NewUint(1000_000), 2, mgr)
		c.Assert(err, IsNil)
	...
	// swap synth --> rune
	{
		...
		amount, _, err := newSwapperV81().Swap(ctx, mgr.Keeper(), tx, common.RuneAsset(), addr, cosmos.ZeroUint(), "", "", nil, cosmos.NewUint(1000_000), 2, mgr)
		c.Assert(err, IsNil)
		c.Check(amount.Uint64(), Equals, swapResult.Uint64(),
			Commentf("Actual: %d Exp: %d", amount.Uint64(), swapResult.Uint64()))
		pool, err = mgr.Keeper().GetPool(ctx, common.BNBAsset)
...
func (s *SwapV81Suite) TestSynthSwap_AssetSynth(c *C) {
	...
	amount, _, err = newSwapperV81().Swap(ctx, mgr.Keeper(), tx1, common.BTCAsset, addr, cosmos.ZeroUint(), "", "", nil, cosmos.NewUint(1000_000_000_000), 2, mgr)
	c.Assert(err, NotNil)
