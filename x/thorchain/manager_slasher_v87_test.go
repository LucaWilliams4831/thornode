...
type SlashingV87Suite struct{}

var _ = Suite(&SlashingV87Suite{})

func (s *SlashingV87Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV87Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV87(k, NewDummyEventMgr())
...
func (s *SlashingV87Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV87(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV87Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV87Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV87Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV87Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV87Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV87Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV87(mgr.Keeper(), mgr.EventMgr())
